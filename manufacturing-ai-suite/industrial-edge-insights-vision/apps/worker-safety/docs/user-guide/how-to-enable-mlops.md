...
3. Restart the model with a new model from Model Registry.
    ...
    ```sh
    curl 'http://<HOST_IP>:8080/pipelines/user_defined_pipelines/worker_safety_mlops/{instance_id_of_currently_running_pipeline}/models' \
    --header 'Content-Type: application/json' \
    --data '{
    "project_name": "worker-safety",
    "version": "v1",
    "category": "Detection",
    "architecture": "YOLO",
    "precision": "fp32",
...
1.  Set up the sample application to start a pipeline. A pipeline named `worker_safety_mlops` is already provided in the `pipeline-server-config.json` for this demonstration with the Worker Safety sample app.
...
2. Bring up the containers
    ...
    docker compose up -d
    ```
3. Check to see if the pipeline is loaded is present which in our case is `worker_safety_mlops`.
    ```sh
    ./sample_list.sh
    ```
4. Modify the payload in `apps/worker-safety/payload.json` to launch an instance for the mlops pipeline
    ```json
    [
        {
            "pipeline": "worker_safety_mlops",
            "payload":{
                "source": {
...
5. Start the pipeline with the above payload.
    ```
    ./sample_start.sh -p worker_safety_mlops
    ```
...
### Upload a model to Model Registry
   ...
   > For this demonstration we will be using Geti trained worker safety model. Usually, the newer model is the same as older, architecture wise, but is retrained for better performance. We will using the same model and call it a different version.

1.  Download and prepare the model.
    ```sh
    export MODEL_URL='https://github.com/open-edge-platform/edge-ai-suites/raw/9b679287cb6650619b4d1dd01f993ae793f8ec04/manufacturing-ai-suite/industrial-edge-insights-vision/worker_safety.zip'
