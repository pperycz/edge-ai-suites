...
3. Update the OPC UA `variable` to appropriate value for the pipeline `worker_safety_opcua` in `configs/config.json` for docker or `helm/config.json` for helm.
...
5. Start the pipeline with the following cURL command. Ensure to give the correct path to the model as seen below. This example starts an AI pipeline.
   ...
   **Note: Update the port to `30107` for helm or `8080` if you are using docker environment**
   ...
   ```sh
    curl http://<HOST_IP>:<port>/pipelines/user_defined_pipelines/worker_safety_opcua -X POST -H 'Content-Type: application/json' -d '{
        "source": {
